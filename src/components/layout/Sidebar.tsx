...
import { communities, myJoinedCommunityIds, myCreatedCommunityIds } from "../../data/mock";
...
const joinedCommunities = myJoinedCommunityIds
  .map((id) => communities.find((c) => c.id === id))
  .filter((c): c is NonNullable<typeof c> => !!c);

const createdCommunities = myCreatedCommunityIds
  .map((id) => communities.find((c) => c.id === id))
  .filter((c): c is NonNullable<typeof c> => !!c);
...
const nav: NavItem[] = [
  { key: "discover", to: "/discover", label: "Discover", icon: faCompass },
  {
    ...
    label: "Komunitas",
    icon: faUserGroup,
    children: joinedCommunities.map((c) => ({ to: `/community/${c.id}`, label: c.name })),
  },
  {
    key: "creator",
    label: "Dashboard Creator",
    icon: faChartLine,
    children: createdCommunities.map((c) => ({ to: `/creator/dashboard/${c.id}`, label: c.name })),
  },
